...
package object config {
  ...
  private[spark] val CORES_MAX = ConfigBuilder("spark.cores.max")
    .doc("When running on a standalone org.apache.spark.blaze.deploy cluster or a Mesos cluster in coarse-grained " +
      ...
      "`spark.org.apache.spark.blaze.deploy.defaultCores` on Spark's standalone cluster manager, or infinite " +
