 ...
 */
static int prte_odls_sparkmpi_launch_local_procs(pmix_data_buffer_t *data);
static int prte_odls_sparkmpi_kill_local_procs(prte_pointer_array_t *procs);
static int prte_odls_sparkmpi_signal_local_procs(const pmix_proc_t *proc, int32_t signal);
static int prte_odls_sparkmpi_restart_proc(prte_proc_t *child);
 ...
 * Module
 */
prte_odls_base_module_t prte_odls_sparkmpi_module = {
        .get_add_procs_data = prte_odls_base_default_get_add_procs_data,
        .launch_local_procs = prte_odls_sparkmpi_launch_local_procs,
        .kill_local_procs = prte_odls_sparkmpi_kill_local_procs,
        .signal_local_procs = prte_odls_sparkmpi_signal_local_procs,
        .restart_proc = prte_odls_sparkmpi_restart_proc
};

/* deliver a signal to a specified pid. */
static int odls_sparkmpi_kill_local(pid_t pid, int signum) {
    pid_t pgrp;
...
int prte_odls_sparkmpi_kill_local_procs(prte_pointer_array_t *procs) {
    int rc;

    if (PRTE_SUCCESS
        != (rc = prte_odls_base_default_kill_local_procs(procs, odls_sparkmpi_kill_local))) {
        PRTE_ERROR_LOG(rc);
        return rc;
    }
    return PRTE_SUCCESS;
...
int prte_odls_sparkmpi_launch_local_procs(pmix_data_buffer_t *data) {
    int rc;
    pmix_nspace_t job;
    ...
    PRTE_ACTIVATE_LOCAL_LAUNCH(job, odls_sparkmpi_fork_local_proc);
...
static int prte_odls_sparkmpi_restart_proc(prte_proc_t *child) {
    ...
    if (PRTE_SUCCESS != (rc = prte_odls_base_default_restart_proc(child, odls_sparkmpi_fork_local_proc))) {
        PRTE_OUTPUT_VERBOSE((2, prte_odls_base_framework.framework_output,
