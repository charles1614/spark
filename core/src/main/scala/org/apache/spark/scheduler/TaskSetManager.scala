  ...
  def stopMPIJobNamespace(): Unit = {
    ...
    if (!split(0).isEmpty) {
      ...
      val th = new Thread() {
        ...
        val filePath = "/tmp/pmixsrv.env"
        Try(new java.io.File(filePath).delete()) match {
          case Success(deleted) =>
            if (deleted) println(s"File $filePath deleted successfully")
            else println(s"Failed to delete file $filePath")
          case Failure(e) =>
            println(s"An exception occurred while deleting file $filePath: $e")
        }
      }
      th.start()
      try {
