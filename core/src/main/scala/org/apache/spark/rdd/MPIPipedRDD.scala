...
import org.apache.spark.{MPIDependency, Partition, SparkEnv, TaskContext}
 ...
 */
private[spark] class MPIPipedRDD[T: ClassTag](
  prev: RDD[T],
  f: (TaskContext, Int, Iterator[T]) => Iterator[String],
  command: Seq[String],
...
) extends MPIMapPartitionsRDD[String, T](prev,
  f,
  ...
  override def compute(split: Partition, context: TaskContext): Iterator[String] = {
    ...
    val iter = itermid.map(x => x._2.toString)
    //    iter

    // append the parent partition to the command
    // use prev_rdd replace firstParent[T]
    val firstParentRdd = dependencies.head.asInstanceOf[MPIDependency[T]].prev_rdd
    var newCommand = command
    for (elem <- firstParentRdd.iterator(split, context)) {
      elem.toString.split(" ").foreach(x => newCommand = newCommand :+ x)
    }

    // pipe
    val pb = new ProcessBuilder(newCommand.asJava).inheritIO()
    ...
    val proc = pb.start() // Return a process object
    val childThreadException = new AtomicReference[Throwable](null)

    // Start a thread to print the process's stderr to ours
    val stderrReaderThread = new Thread(s"${
      PipedRDD.STDERR_READER_THREAD_PREFIX
    } $command") {
    ...
    //    val stdinWriterThread = new Thread(s"${
    //      PipedRDD.STDIN_WRITER_THREAD_PREFIX
    //    } $command") {
    //      override def run(): Unit = {
    ...
    // TODO: context hook not work, should dig in, temp use shutdownHook
    ...
    // Return an iterator that read lines from the process's stdoupipedrddt
        ...
        val result = if (lines.hasNext) {
          ...
          if (exitStatus != 0) {
            throw new IllegalStateException(s"Subprocess exited with status $exitStatus. " +
              s"Command ran: " + newCommand.mkString(" "))
          }
          false
        }
