...
abstract class RDD[T: ClassTag](
  ...
  def mpipipe(
    command: Seq[String],
    f: T => String = (iter => iter.toString), // (TaskContext, partition index, iterator)
    env: Map[String, String] = Map(),
    printPipeContext: (String => Unit) => Unit = null,
    printRDDElement: (T, String => Unit) => Unit = null,
    separateWorkingDir: Boolean = false,
    ...
    val pipedrdd = new MPIPipedRDD[T](this,
      (_, _, iter) => iter.map(f),
      command, env,
      if (printPipeContext ne null) sc.clean(printPipeContext) else null,
      if (printRDDElement ne null) sc.clean(printRDDElement) else null,
  ...
  def mpimap[U: ClassTag](f: T => U): RDD[U] = withScope {
    ...
    val cleanF = sc.clean(f)
    val mpimprdd = new MPIMapPartitionsRDD[U, T](
      this,
      (_, _, iter) => iter.map(cleanF), isFromBarrier = true)

    mpimprdd
  }

  def mpimapPartitions[U: ClassTag](
    f: Iterator[T] => Iterator[U],
    preservesPartitioning: Boolean = false): RDD[U] = withScope {
    val cleanedF = sc.clean(f)
