...
#include "rmaps_blaze.h"
#include "rmaps_blaze_lex.h"
#include "src/runtime/prte_globals.h"
#include "src/util/show_help.h"

static int prte_rmaps_rf_map(prte_job_t *jdata);

prte_rmaps_base_module_t prte_rmaps_blaze_module = {.map_job = prte_rmaps_rf_map};

static int prte_rmaps_blaze_parse(const char *);
static int prte_rmaps_blaze_parse_args(const char *);
static char *prte_rmaps_blaze_parse_string_or_int(void);
char *prte_rmaps_blaze_slot_list = NULL;
...
/*
 * Create a blaze  mapping for the job.
 */
static int prte_rmaps_rf_map(prte_job_t *jdata)
{
    prte_job_map_t *map;
    ...
    int32_t num_slots;
    prte_rmaps_blaze_map_t *rfmap;
    int32_t relative_index, tmp_cnt;
    int rc;
    prte_proc_t *proc;
    prte_mca_base_component_t *c = &prte_rmaps_blaze_component.super.base_version;
    ...
    if (PRTE_FLAG_TEST(jdata, PRTE_JOB_FLAG_RESTART)) {
        prte_output_verbose(5, prte_rmaps_base_framework.framework_output,
                            "mca:rmaps:rf: job %s being restarted - blaze cannot map",
    ...
    if (NULL != jdata->map->req_mapper
        ...
        prte_output_verbose(5, prte_rmaps_base_framework.framework_output,
                            "mca:rmaps:rf: job %s not using blaze mapper",
                            PRTE_JOBID_PRINT(jdata->nspace));
        return PRTE_ERR_TAKE_NEXT_OPTION;
    }
    if (PRTE_MAPPING_BYUSER != PRTE_GET_MAPPING_POLICY(jdata->map->mapping)) {
        /* NOT FOR US */
        prte_output_verbose(5, prte_rmaps_base_framework.framework_output,
                            "mca:rmaps:rf: job %s not using blaze policy",
                            PRTE_JOBID_PRINT(jdata->nspace));
        return PRTE_ERR_TAKE_NEXT_OPTION;
    }
    if (PRTE_BIND_ORDERED_REQUESTED(jdata->map->binding)) {
        /* NOT FOR US */
        prte_output_verbose(5, prte_rmaps_base_framework.framework_output,
                            "mca:rmaps:rf: job %s binding order requested - blaze cannot map",
                            PRTE_JOBID_PRINT(jdata->nspace));
        return PRTE_ERR_TAKE_NEXT_OPTION;
    }
    if (!prte_get_attribute(&jdata->attributes, PRTE_JOB_FILE, (void **) &blaze, PMIX_STRING)
        || NULL == blaze) {
        /* we cannot do it */
        prte_output_verbose(5, prte_rmaps_base_framework.framework_output,
                            "mca:rmaps:rf: job %s no blaze specified",
                            PRTE_JOBID_PRINT(jdata->nspace));
        return PRTE_ERR_BAD_PARAM;
    }

    prte_output_verbose(5, prte_rmaps_base_framework.framework_output,
                        "mca:rmaps:blaze: mapping job %s", PRTE_JOBID_PRINT(jdata->nspace));
    ...
    if (0 == app->num_procs && 1 < jdata->num_apps) {
        prte_show_help("help-rmaps_blaze.txt", "prte-rmaps-rf:multi-apps-and-zero-np", true,
                       jdata->num_apps, NULL);
        rc = PRTE_ERR_SILENT;
        goto error;
    }
    ...
    /* parse the blaze, storing its results in the rankmap */
    if (PRTE_SUCCESS != (rc = prte_rmaps_blaze_parse(blaze))) {
        rc = PRTE_ERR_SILENT;
        goto error;
    }
    ...
    for (i = 0; i < jdata->apps->size; i++) {
        ...
        if (0 == app->num_procs) {
            /* set the number of procs to the number of entries in that blaze */
            app->num_procs = num_ranks;
        }
        if (0 == app->num_procs) {
            prte_show_help("help-rmaps_blaze.txt", "bad-syntax", true, blaze);
            rc = PRTE_ERR_SILENT;
            goto error;
        }
        for (k = 0; k < app->num_procs; k++) {
            rank = vpid_start + k;
            /* get the blaze entry for this rank */
            if (NULL
                == (rfmap = (prte_rmaps_blaze_map_t *) prte_pointer_array_get_item(&rankmap,
                                                                                       rank))) {
                ...
                if (NULL != jobslots) {
                    ...
                    prte_show_help("help-rmaps_blaze.txt", "missing-rank", true, rank,
                                   blaze);
                    rc = PRTE_ERR_SILENT;
                    goto error;
                }
                ...
                if (0 == strlen(rfmap->slot_list)) {
                    ...
                    prte_show_help("help-rmaps_blaze.txt", "no-slot-list", true, rank,
                                   rfmap->node_name);
                    rc = PRTE_ERR_SILENT;
                    goto error;
                }
                ...
                PRTE_LIST_FOREACH(nd, &node_list, prte_node_t)
                {
                    if (NULL != rfmap->node_name && 0 == strcmp(nd->name, rfmap->node_name)) {
                        ...
                        if (relative_index >= (int) prte_list_get_size(&node_list)
                            || (0 > relative_index)) {
                            prte_show_help("help-rmaps_blaze.txt", "bad-index", true,
                                           rfmap->node_name);
            ...
            if (NULL == node) {
                prte_show_help("help-rmaps_blaze.txt", "bad-host", true, rfmap->node_name);
                rc = PRTE_ERR_SILENT;
                goto error;
            }
...
static int prte_rmaps_blaze_parse(const char *blaze)
{
    int token;
    int rc = PRTE_SUCCESS;
    int cnt;
    ...
    prte_rmaps_blaze_map_t *rfmap = NULL;
    ...
    prte_rmaps_blaze_done = false;
    prte_rmaps_blaze_in = fopen(blaze, "r");

    if (NULL == prte_rmaps_blaze_in) {
        prte_show_help("help-rmaps_blaze.txt", "no-blaze", true, blaze);
        rc = PRTE_ERR_NOT_FOUND;
        goto unlock;
    }

    while (!prte_rmaps_blaze_done) {
        token = prte_rmaps_blaze_lex();
        ...
        case PRTE_RANKFILE_QUOTED_STRING:
            prte_show_help("help-rmaps_blaze.txt", "not-supported-blaze", true,
                           "QUOTED_STRING", blaze);
            rc = PRTE_ERR_BAD_PARAM;
            PRTE_ERROR_LOG(rc);
            goto unlock;
        ...
        case PRTE_RANKFILE_RANK:
            ...
            if (PRTE_RANKFILE_INT == token) {
                rank = prte_rmaps_blaze_value.ival;
                rfmap = PRTE_NEW(prte_rmaps_blaze_map_t);
        ...
        case PRTE_RANKFILE_USERNAME:
            prte_show_help("help-rmaps_blaze.txt", "not-supported-blaze", true, "USERNAME",
                           blaze);
            rc = PRTE_ERR_BAD_PARAM;
            PRTE_ERROR_LOG(rc);
            goto unlock;
        case PRTE_RANKFILE_EQUAL:
            ...
            case PRTE_RANKFILE_RELATIVE:
                if (PRTE_RANKFILE_INT == token) {
                    sprintf(buff, "%d", prte_rmaps_blaze_value.ival);
                    value = buff;
                } else {
                    value = prte_rmaps_blaze_value.sval;
                }
        ...
        case PRTE_RANKFILE_SLOT:
            if (NULL == node_name || rank < 0
                || NULL == (value = prte_rmaps_blaze_parse_string_or_int())) {
                prte_show_help("help-rmaps_blaze.txt", "bad-syntax", true, blaze);
                rc = PRTE_ERR_BAD_PARAM;
                PRTE_ERROR_LOG(rc);
                goto unlock;
            ...
            if (NULL != prte_pointer_array_get_item(assigned_ranks_array, rank)) {
                prte_show_help("help-rmaps_blaze.txt", "bad-assign", true, rank,
                               prte_pointer_array_get_item(assigned_ranks_array, rank), blaze);
                rc = PRTE_ERR_BAD_PARAM;
                free(value);
                goto unlock;
            } else {
    ...
    fclose(prte_rmaps_blaze_in);
    prte_rmaps_blaze_lex_destroy();
...
static char *prte_rmaps_blaze_parse_string_or_int(void)
{
    ...
    if (PRTE_RANKFILE_EQUAL != prte_rmaps_blaze_lex()) {
        return NULL;
    }

    rc = prte_rmaps_blaze_lex();
    switch (rc) {
    case PRTE_RANKFILE_STRING:
        return strdup(prte_rmaps_blaze_value.sval);
    case PRTE_RANKFILE_INT:
        sprintf(tmp_str, "%d", prte_rmaps_blaze_value.ival);
